...
CREATE OR REPLACE FUNCTION auth.validate_reset_token(p_token text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  v_user_id := auth.validate_reset_token(p_token);
...
GRANT EXECUTE ON FUNCTION auth.reset_password TO anon, authenticated;
